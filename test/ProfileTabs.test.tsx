...
describe("ProfileTabs", () => {
  ...
  describe("❌ Tab Data Validation (Currently Failing)", () => {
    it("should handle null tabs gracefully", () => {
      const { container } = render(
        <ProfileTabs tabs={null as any} />
      );
    ...
    it("should handle undefined tabs gracefully", () => {
      const { container } = render(
        <ProfileTabs tabs={undefined as any} />
      );
      expect(container).toBeInTheDocument();
    });
    ...
    it("should handle tabs with missing labels", () => {
      const tabs: any[] = [
        { id: "tab1", label: "" },
        { id: "tab2", label: "Personal" },
      ];
      render(
    ...
    it("should handle tabs with undefined content", () => {
      const tabs: ProfileTabsProps["tabs"] = [
        { id: "tab1", label: "Tab 1", content: undefined },
      ];
      render(
        <ProfileTabs tabs={tabs} />
      );
    ...
    it("should handle tabs with duplicate ids", () => {
      const tabs: ProfileTabsProps["tabs"] = [
  ...
  // ❌ FAILING TESTS - Fields rendering
  describe("❌ Fields Rendering (Currently Failing)", () => {
    it("should render fields with custom renderer", () => {
      const tabs: ProfileTabsProps["tabs"] = [
        {
          id: "personal",
          label: "Personal",
  ...
  // ❌ FAILING TESTS - Tab switching
  describe("❌ Tab Switching Validation (Currently Failing)", () => {
    it("should validate active tab exists", () => {
      const tabs: ProfileTabsProps["tabs"] = [
        { id: "tab1", label: "Tab 1" },
        { id: "tab2", label: "Tab 2" },
      ];
      render(
        <ProfileTabs tabs={tabs} defaultTab="invalid" />
      );
    ...
    it("should handle switching to non-existent tab", () => {
      ...
      const { container } = render(
        <ProfileTabs tabs={tabs} onTabChange={handleTabChange} />
      );
      expect(container).toBeInTheDocument();
    });
  });

  // ❌ FAILING TESTS - Callback handling
  describe("❌ Callback Handling (Currently Failing)", () => {
    it("should call onTabChange with correct tab id", async () => {
      ...
      fireEvent.click(dematTab);
      expect(handleTabChange).toHaveBeenCalledWith("demat");
    });

    it("should handle null onTabChange callback gracefully", () => {
      const tabs: ProfileTabsProps["tabs"] = [
        { id: "tab1", label: "Tab 1" },
        { id: "tab2", label: "Tab 2" },
      ];
      const { container } = render(
        <ProfileTabs tabs={tabs} onTabChange={null as any} />
      );
  ...
  // ❌ FAILING TESTS - Mobile responsiveness
  describe("❌ Mobile Responsiveness (Currently Failing)", () => {
    it("should apply mobile styles below breakpoint", () => {
      const tabs: ProfileTabsProps["tabs"] = [
    ...
    it("should scroll horizontally on mobile for many tabs", () => {
      const tabs: ProfileTabsProps["tabs"] = Array.from({ length: 8 }, (_, i) => ({
        id: `tab${i}`,
        label: `Tab ${i + 1}`,
      }));
      const { container } = render(
